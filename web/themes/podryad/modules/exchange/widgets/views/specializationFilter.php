                    ...
                    <div class="filter__item">
                        ...
                        <div class="filter__list">
                            <?php foreach ($part2->children() as $part3): ?>
                                <div class="filter-item<?= in_array($part3->id,
                                    $checked) ? ' active' : '' ?>"><?= $part3->title ?>
                                    <input type="checkbox" name="<?= $filterModel ?>[specializationIds][]"
                                           value="<?= $part3->id ?>" <?= in_array($part3->id,
        ...
        <?php else: ?>
            <?php foreach ($model->children() as $part1): ?>
                <div class="filter__item">
                    ...
                    <div class="filter__list">
                        <?php foreach ($part1->children() as $part2): ?>
                            <div class="filter-item<?= in_array($part2->id,
                                $checked) ? ' active' : '' ?>"><?= $part2->title ?>
                                <input type="checkbox" name="<?= $filterModel ?>[specializationIds][]"
                                       value="<?= $part2->id ?>" <?= in_array($part2->id,
                                    $checked) ? 'checked="checked"' : '' ?>>
                            </div>
                        <?php endforeach ?>
                    </div>
        ...
        <?php if ($minMaxPrice): ?>
            <div class="filter-cost">
                ...
                <div class="filter-cost-value">
                    <input name="min" type="hidden" value="<?= $minMaxPrice['max'] ?>" id="cost-line-max"
                           disabled="disabled"/>
                    <input name="max" type="hidden" value="<?= $minMaxPrice['min'] ?>" id="cost-line-min"
                           disabled="disabled"/>
                    <input type="text" value="<?= $tender->priceMin ?>" id="cost-line-value-max"
                           class="filter-cost-value__input" name="<?= $filterModel ?>[priceMin]"><span>—</span>
                    <input type="text" value="<?= $tender->priceMax ?>" id="cost-line-value-min"
                           class="filter-cost-value__input"
